...
/-- Collision resistance of a bus commitment `H`, packaged as a
`HardnessAssumption`: a break is a colliding pair (distinct preimages with equal
digest). A reduction to this `HardnessAssumption` returns such a pair as its
break — e.g. the segment/bus layer built in Issue 5. A consumer discharges the
break branch from `(crAssumption H).Holds` ("no colliding pair exists"), which is
exactly `CollisionResistant H` (injectivity of `hash`).

Paper: `def:bus-cr` / `Adv^cr`. -/
def crAssumption (H : HashCommitment) : HardnessAssumption where
  Break := H.Domain × H.Domain
  IsBreak := fun bb => bb.1 ≠ bb.2 ∧ H.hash bb.1 = H.hash bb.2
