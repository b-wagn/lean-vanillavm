...
The protocol: a value is committed as `d = hash w`, and a proof is a *pair* of
...
the common value is a valid witness; if they disagree, they are a collision.

## Main results
* `twoInspectors_extract_or_collision` — the two-inspector system `ReducesTo`
  collision resistance, so `ReducesTo` is satisfiable (not `False`).
...
variable (H : HashCommitment) (P Q : H.Domain → Prop)
...
@[reducible] def twoInspectorsRel : Relation where
  Stmt := H.Digest
  Wit := H.Domain
  rel := fun d w => H.hash w = d ∧ P w ∧ Q w

/-- A proof is a pair of openings: inspector 1 checks `P`, inspector 2 checks
`Q`. Nothing forces the two openings to be equal. -/
@[reducible] def twoInspectorsAS : ArgumentSystem (twoInspectorsRel H P Q) where
  Proof := H.Domain × H.Domain
  verify := fun d ab =>
    (H.hash ab.1 = d ∧ P ab.1) ∧
    (H.hash ab.2 = d ∧ Q ab.2)

/-- The two-inspector system reduces to collision resistance: the extractor
returns opening 1, the reduction returns the pair. -/
theorem twoInspectors_extract_or_collision :
    Reduction.ReducesTo (twoInspectorsAS H P Q) (Reduction.crAssumption H) := by
...
/-- Assuming collision resistance, the two-inspector system is knowledge-sound. -/
theorem twoInspectors_knowledgeSound (hcr : CollisionResistant H) :
    KnowledgeSound (twoInspectorsAS H P Q) := by
  obtain ⟨E, B, hEB⟩ := twoInspectors_extract_or_collision H P Q
  exact Reduction.knowledgeSound_of_extractOrBreak hEB fun bb hbb =>
    hbb.1 (hcr bb.1 bb.2 hbb.2)
