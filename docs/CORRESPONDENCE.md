...
| Collision resistance (`Adv^cr`) | `VanillaZkVM.CollisionResistant` (injectivity) | provisional | proved | — | — | _unreviewed (Issue 2 re-sign)_ |
...
> change; `CollisionResistant` is modeled as injectivity of `hash` (an
> idealization).
...
| collision assumption (`Adv^cr`) | `VanillaZkVM.Reduction.crAssumption` | — | proved | — | — | _unreviewed_ |
| CR non-vacuity (injective model) | `VanillaZkVM.collisionResistant_idHashCommitment` | — | proved | — | — | _unreviewed_ |
