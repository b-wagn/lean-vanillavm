...
| Collision resistance (`Adv^cr`) | `VanillaZkVM.CollisionResistant` (keyed, finder-based) | provisional | proved | — | — | _unreviewed (Issue 2 re-sign)_ |
...
> change; `CollisionResistant` is now keyed and finder-based, with the efficiency
> and probability layers deferred to Issue 6.
...
| CR ⟺ injectivity (non-vacuity) | `VanillaZkVM.collisionResistant_iff_injective` / `collisionResistant_idHashCommitment` | — | proved | — | — | _unreviewed_ |
